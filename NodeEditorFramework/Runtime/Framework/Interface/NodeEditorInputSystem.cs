	...
	public static class NodeEditorInputSystem
	{
		...
		private static void HandleContextClicks (NodeEditorInputInfo inputInfo)
		{
			...
			{ // Handle context clicks on Node and canvas
				ContextType contextType = inputInfo.editorState.focusedNode != null ? ContextType.Node : ContextType.Canvas;
				if (!inputInfo.editorState.canvas.CanOpenContextMenu(contextType)) return;
				GenericMenu contextMenu = new GenericMenu();
				FillContextMenu (inputInfo, contextMenu, contextType);
				if (contextMenu.ItemCount == 0) return;
